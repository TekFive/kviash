...
class JavaxRequestAdapter(val servletRequest: HttpServletRequest) : HttpRequestSource {
    ...
    override fun getSession(createIfNotExists: Boolean): HttpSession? {
        return servletRequest.getSession(createIfNotExists)?.let { JavaxSessionAdapter(it) }
    }
